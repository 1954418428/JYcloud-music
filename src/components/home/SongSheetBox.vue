  ...
  <div class="song-sheet-list">
    <div class="DailySongsItem">
      <router-link class="img-a" to="/playlist?id=daily">
        <img
          v-lazy="{
            src: require('@/assets/img/rili/' + dailySongsItem.day + '.png'),
...
export default {
  ...
  props: {},
  computed: mapState("UserModule", ["userState"]),
  watch: {
    "userState.isLogin": {
      async handler() {
        const personalizedPlayList = JSON.parse(
          sessionStorage.getItem(
            `${this.userState.isLogin}:PersonalizedPlayList`
          )
        );
        ...
        if (personalizedPlayList && personalizedPlayList.length != 0) {
          ...
          sessionStorage.setItem(
            `${this.userState.isLogin}:PersonalizedPlayList`,
            JSON.stringify(this.PersonalizedPlayList)
          );
        }
