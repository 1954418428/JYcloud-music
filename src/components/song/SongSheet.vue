  ...
  <div class="song-sheet">
    ...
    <div class="sheet-table-wrap">
      <el-table
        ...
        ref="sheettable"
      >
        <el-table-column label="操作" :width="c1">
          <template slot-scope="scope">
            <div class="operation">
              <span> {{ scope.$index + 1 }}</span>
              <i
                class="iconfont icon-bofang"
                @click.stop="btnBf(scope.$index)"
                title="播放"
              ></i>
        ...
        <el-table-column label="歌曲标题" show-overflow-tooltip :width="c2">
          <template slot-scope="scope">
            {{ scope.row.name }}
            <a
              v-if="scope.row.mv"
              href="javascript:void(0)"
              @click.prevent="gotoMv(scope.row.mv)"
              style="
                font-size: 18px;
                position: relative;
                top: 1px;
        ...
        <el-table-column label="时间" :width="c3">
          <template slot-scope="scope">
            {{ formatTime(scope.row.dt / 1000) }}
          </template>
        </el-table-column>

        <el-table-column label="歌手" :width="c4" show-overflow-tooltip>
          <template slot-scope="scope">
        ...
        <el-table-column label="专辑" :width="c5" show-overflow-tooltip>
          <template slot-scope="scope">
...
export default {
  ...
  watch: {},
  methods: {
    ...mapMutations("PlayerModule", ["pause"]),
          ...
          sheetApi
            ...
            .then((res) => {
              ...
              this.catchStorage.removeCatchItem(
                `playlistDetail:${this.$route.query.id}`
              );
              this.catchStorage.removeCatchItem(
                `playlistSongs:${this.$route.query.id}`
              );
            });
        })
        .catch(() => {});
