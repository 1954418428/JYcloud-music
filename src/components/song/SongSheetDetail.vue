  ...
  <div class="song-sheet-detail">
    ...
    <div class="sheet-info-wrap">
      ...
      <div class="user" v-if="playListType != '专辑' && playListDetail.creator">
        <img
          :src="playListDetail.creator.avatarUrl + '?param=208y208'"
          alt=""
        />
        <router-link
          :to="`/user/home?id=${playListDetail.creator.userId}`"
          style="color: #0c73c2"
          >{{ playListDetail.creator.nickname }}</router-link
        >
        <span
          >{{
            moment(parseInt(playListDetail.createTime)).format("YYYY-MM-DD") +
            (playListType == "歌单" ? " 创建" : " 发布")
          }}
        </span>
      </div>
      <div v-else>
        歌手：<router-link
          style="color: #0c73c2"
          :to="`/singer?id=${ar.id}`"
          v-for="(ar, index) in playListDetail.artists"
          :key="ar.id"
          >{{ index != 0 ? " / " : "" }}{{ ar.name }}</router-link
        >
      ...
      <div class="operation">
        ...
        <a
          href="javascript:void(0)"
          class="collect"
          @click.prevent="collectClicked"
          :style="
            playListDetail.subscribed ? 'background-color:rgb(231,231,231)' : ''
          "
        >
          <i class="iconfont icon-shoucang"></i>
          {{ playListDetail.subscribed ? "已收藏" : "收藏" }}

          <span v-if="playListDetail.subscribedCount"
            >({{ playListDetail.subscribedCount }})</span
          >
        </a>
      ...
      <div
        class="tags"
        v-if="playListDetail.tags && playListDetail.tags.length"
      >
        <span> 标签： </span>
        <router-link
          :to="`/discoverplaylist?cat=${item}`"
      ...
      <div v-if="playListDetail.company">
        <span> 发行公司：{{ playListDetail.company }} </span>
      </div>
      <div class="count">
        <span> 歌曲：{{ playListDetail.trackCount }} 首 </span>
  ...
  methods: {
    ...
    collectClicked() {
      if (this.playListDetail.subscribed) {
        //取消收藏
        sheetApi.collectPlayList(this.playListDetail.id, 2).then((res) => {
        ...
        //收藏
        sheetApi
          .collectPlayList(this.playListDetail.id, 1)
          .then((res) => {
