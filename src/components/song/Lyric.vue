  ...
  <div class="lyric-box" ref="lyricBox">
    <div class="left-box">
      <div class="left-top">
        ...
        >
          <img src="../../assets/img/唱片.png" alt="" class="changpian-img" />
          <img
            v-if="audio.album.picUrl"
      ...
      <ul ref="lyricUL" class="lyric-ul" id="lyric-ul">
        ...
        >
          ...
          <i
            ...
            :title="'跳转到' + item.oldTime"
            @click="tiaozhuan(index, item.time)"
          >
          </i>
        </li>
      </ul>
...
export default {
  ...
  props: {},
  computed: mapState("PlayerModule", ["audio", "player"]),
  watch: {
    "audio.id": {
      handler() {
        if (this.audio.id == 0) {
    ...
    "audio.currentTime": {
      handler() {
        ...
        for (let i = 0; i < this.lyricArr.length; i++) {
          ...
          if (
            this.lyricArr[i].time >
            this.audio.currentTime + this.offsetTime
          ) {
            // console.log("高亮",i-1);
            ...
            if (
              this.$refs.lyricLI[i - 1].dataset.index != this.lyricActiveIndex
            ) {
              ...
              const move =
                this.lyricActiveIndex * this.lyricLiHeight -
                4 * this.lyricLiHeight;
  ...
  methods: {
    ...mapMutations("PlayerModule", ["play", "pause", "changeCurrentTime"]),
    //旋转唱片被单击
    cpClicked() {
    ...
    tiaozhuan(index, time) {
      ...
      const move =
        this.lyricActiveIndex * this.lyricLiHeight - 5 * this.lyricLiHeight;
      this.$refs.lyricUL.scrollTo(0, move);
