<template>
  <div class="bofang-list">
    ...
    <ul>
      <li
        v-for="(item, index) in player.songs"
        :key="item.id"
        @dblclick="asyncUpdateAudio({ currentIndex: index })"
      >
        <div
          class="c1"
          :style="
            index == player.currentIndex
              ? 'color:rgb(236,65,65);font-weight: 700;'
              : ''
          "
        >
          ...
          <a
            ...
            @click.prevent="gotoSongplaying(index, item.id)"
            :title="item.name"
          >
            {{ item.name }}
          </a>
        ...
        <div class="c2">
          <i
            ...
            title="移出列表"
            @click="asyncRemoveSong({ removeIndex: index })"
          ></i>
          <i class="iconfont icon-xiazai"></i>
          <i
            class="iconfont icon-shoucang"
            @click="collectClicked({ tracks: [item.id] })"
            title="收藏音乐"
          ></i>
        </div>
        <div
...
export default {
  ...
  props: {},
  computed: mapState("PlayerModule", ["player"]),
  watch: {},
  methods: {
