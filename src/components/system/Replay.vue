<template>
  <div class="reply-item-box" id="reply-box" ref="replyBox">
    ...
    <div style="width: 100%">
      <textarea
        class="input-conntent"
        v-model="content"
        rows="3"
        :placeholder="commentQueryData.msg"
      >
      </textarea>
      <span class="btn" @click="sendComment"> 发表评论 </span>
      <span
        class="btn"
        @click="deleteCommentId"
...
export default {
  ...
  computed: {
    ...mapState("UserModule", ["userState", "userInfo"]),
    ...mapState("CommentModule", ["commentQueryData"]),
    content: {
      get() {
  ...
  watch: {},
  methods: {
    ...mapMutations("CommentModule", ["updateContent", "deleteCommentId"]),
          ...
          systemApi
            ...
            .then((res) => {
              ...
              if (res.code == 200) {
                ...
                if (
                  res.comment.beRepliedUser &&
                  res.comment.beRepliedUser.userId
                ) {
                  let obj = {};
                  obj.beRepliedCommentId =
                    this.commentQueryData.beRepliedCommentId;
