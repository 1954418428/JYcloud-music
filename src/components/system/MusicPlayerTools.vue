  ...
  <div>
    <div class="top">
      ...
      <div class="music-operation">
        <i
          class="pre-btn iconfont icon-shangyishoushangyige"
          title="上一首"
          @click="cutSong({ type: 'pre' })"
        ></i>

        <i
          v-if="!player.playing"
          class="bofang-btn iconfont icon-bofang"
          @click="play"
          title="播放"
        ></i>
        <i
          v-else
          class="zangting-btn iconfont icon-zanting"
          @click="pause"
          title="暂停"
        ></i>

        <i
          class="next-btn iconfont icon-xiayigexiayishou"
          @click="cutSong({ type: 'next' })"
          title="下一首"
        ></i>
      </div>
    </div>
    ...
    <div class="tool">
      ...
      <div class="m-t">
        <span class="music-volume">
          <i
            class="iconfont icon-volumedown"
            @click="updateVolume({ type: 'down' })"
            title="减音"
          ></i>
          {{ player.volume }}
          <i
            class="iconfont icon-volumeup"
            @click="updateVolume({ type: 'up' })"
            title="增音"
          ></i>

          <i
            class="iconfont icon-shengyinjingyin"
            @click="updateVolume({ type: 'muted' })"
            title="静音"
        ...
        <span class="play-type">
          <i
            class="iconfont icon-shuzishunxu"
            title="顺序"
            v-show="player.playType == 0"
            @click="updatePlayType"
          ></i>
          <i
            class="iconfont icon-24gl-shuffle"
            title="随机"
            v-show="player.playType == 1"
            @click="updatePlayType"
          ></i>
          <i
            class="iconfont icon-xunhuan"
            title="循环"
            v-show="player.playType == 2"
            @click="updatePlayType"
          ></i>
          <i
            class="iconfont icon-danquxunhuan"
            title="单曲循环"
            v-show="player.playType == 3"
            @click="updatePlayType"
          ></i>
        </span>
        <span class="op">
          ...
          <i
            ...
            title="收藏"
            @click="collectClicked({ tracks: [audio.id] })"
          ></i>
...
export default {
  ...
  props: {},
  computed: mapState("PlayerModule", ["audio", "player", "clyric"]),
  watch: {},
  methods: {
    ...
    ...mapActions("PlayerModule", ["cutSong"]),
