          ...
          <ul class="area">
            <li v-for="val in areaData" :key="val">
              <router-link
                :to="{
                  path: '/discovermv',
                  query: { area: val, type: page.type, order: page.order }
                }"
                :class="page.area == val ? 'active' : ''"
                >{{ val }}</router-link
              >
            </li>
          </ul>
          ...
          <ul class="type">
            <li v-for="val in typeData" :key="val">
              <router-link
                :to="{
                  path: '/discovermv',
                  query: { area: page.area, type: val, order: page.order }
                }"
                :class="page.type == val ? 'active' : ''"
                >{{ val }}</router-link
              >
            </li>
          </ul>
          ...
          <ul class="order">
            <li v-for="val in orderData" :key="val">
              <router-link
                :to="{
                  path: '/discovermv',
                  query: { area: page.area, type: page.type, order: val }
                }"
                :class="page.order == val ? 'active' : ''"
                >{{ val }}</router-link
              >
            </li>
          </ul>
          ...
          <li v-for="item in mvList" :key="item.id">
            <div class="mv-img">
              <router-link :to="`/mv?id=${item.id}`">
                <img
                  v-lazy="{
                    src: item.cover + '?param=290y160',
                    error: require('@/assets/img/mv.png'),
                    loading: require('@/assets/img/mv.png')
                  }"
                  alt=""
                />
              </router-link>
