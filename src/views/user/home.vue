  ...
  <div class="userhome-box">
    <div class="userhome-wrap">
          ...
          <h2>
            ...
            <span class="sex" v-if="userData.profile.gender">
              <i
                ...
                style="color: rgb(38, 166, 228)"
                v-if="userData.profile.gender == 1"
              ></i>
              <i
                class="iconfont icon-nv"
                style="color: rgb(255, 181, 211)"
                v-else
              ></i>
            </span>
            <!-- <span class="viptype">VIP2</span> -->
            <span class="usertype" v-if="userData.level"
              >Lv{{ userData.level }}</span
            >
          </h2>
          <div class="user-desc">个人简介:{{ userData.profile.signature }}</div>
          <!-- <div class="user-area">
                        所在地区：湖南省-长沙市
                    </div> -->
        </div>
      ...
      <div class="user-record" v-if="CreatedPlaylist.length">
        <h3>
          听歌排行
          <span style="font-size: 14px; margin-right: 12px">最近一周</span>

          <span
            @click="showCount = 10"
            :class="showCount == 10 ? 'count-active' : ''"
            >10首</span
          >
          <span
            @click="showCount = 50"
            :class="showCount == 50 ? 'count-active' : ''"
            >50首</span
          >
          <span
            @click="showCount = 100"
            :class="showCount == 100 ? 'count-active' : ''"
            >100首</span
          >
        </h3>
        <div>
          <SongSheet
            :songs="recordSongs.slice(0, showCount)"
            :playCount="0"
            :trackCount="recordSongs.slice(0, showCount).length"
            :c6IsShow="true"
            :c2="160"
            :c3="120"
            :c4="160"
      ...
      <div class="create-sheet" v-if="CreatedPlaylist.length">
        <h3>
          {{ isMe ? "我" : userData.profile.nickname + "-" }}创建的歌单({{
      ...
      <div class="collect-sheet" v-if="SubPlaylist.length">
        <h3>
          {{ isMe ? "我" : userData.profile.nickname + "-" }}收藏的歌单({{
            SubPlaylist.length
          }})
        </h3>
        <ul>
        ...
        if (this.$route.query.id) {
          ...
          userApi
            ...
            .then((res) => {
              ...
              sheetApi.getUserPlayList(uid, 100).then((res) => {
                this.CreatedPlaylist = res.playlist.slice(0, playlistCount);
                this.SubPlaylist = res.playlist.slice(
                  playlistCount,
                  res.playlist.length
                );
              });
