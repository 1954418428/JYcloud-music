<template>
  <div class="mymusic-box">
    <div class="mymusic-wrap">
      <div class="left-wrap">
        <div>
          ...
          <div class="myplaylist-wrap">
            ...
            <ul>
              <router-link
                :to="`/mymusic/myplaylist?id=${item.id}`"
      ...
      <div style="height: 1px; width: 240px"></div>

      <div class="right-wrap" style="min-height: 960px">
        <router-view :CreatedPlaylistIds="CreatedPlaylistIds"> </router-view>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async initPlaylist() {
      let playlist = this.catchStorage.getCatchItem(
        `userPlayList:${this.userInfo.userId}`
      );
      console.log(playlist);
      if (!playlist) {
        const res = await sheetApi.getUserPlayList(this.userInfo.userId);
        playlist = res.playlist;
        this.catchStorage.setCatchItem(
          `userPlayList:${this.userInfo.userId}`,
          playlist,
          60
        );
      ...
      this.CreatedPlaylist = playlist.slice(
        0,
      ...
      this.SubPlaylist = playlist.slice(
        this.subCount.createdPlaylistCount,
        playlist.length
      );
      if ("/mymusic" == this.$route.fullPath) {
        this.$router.push(
          `/mymusic/myplaylist?id=${this.CreatedPlaylist[0].id}`
        );
      }
    },
    //修改歌单
    updatePlaylist(item) {
      this.asyncupdateCreateBoxIsShow({
        isShow: true,
        model: 2,
        playlist: {
          id: item.id,
        ...
        .then(() => {
          ...
          sheetApi
            ...
            .then((res) => {
              ...
              //删除缓存
              this.catchStorage.removeCatchItem(
                `userPlayList:${this.userInfo.userId}`
              );
              this.catchStorage.removeCatchItem(
                `userSubcount:${this.userInfo.userId}`
              );
  ...
  beforeCreated() {},
  async created() {
    let res = this.catchStorage.getCatchItem(
      `userSubcount:${this.userInfo.userId}`
    );
    if (!res) {
    ...
    const { mvCount, artistCount, createdPlaylistCount, subPlaylistCount } =
      res;
