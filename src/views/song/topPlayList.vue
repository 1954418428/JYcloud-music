    ...
    <div class="topplaylist-wrap">
      <div class="left-wrap">
        <div class="tsb">
          ...
          <ul>
            <li
              ...
              :key="item.id"
              :class="currentId == item.id ? 'avtive' : ''"
              @click="updateTopPlayList(item.id)"
            >
              <div>
                <img
        ...
        <div class="qqb">
          ...
          <ul>
            <li
              v-for="item in topList.slice(4, topList.length)"
              :key="item.id"
              :class="currentId == item.id ? 'avtive' : ''"
              @click="updateTopPlayList(item.id)"
            >
...
export default {
  ...
  methods: {
    ...
    async initTopPlayListData(pid) {
      ...
      await sheetApi
        .getPlayListTrackAll({ id: pid, limit: 1000, offset: 0 })
        .then((res) => {
